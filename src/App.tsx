...
import { getGiaResponse, getGiaAudio, resetGiaSession } from "./services/geminiService";
...
export default function App() {
  ...
  const [messages, setMessages] = useState<ChatMessage[]>(() => {
    const saved = localStorage.getItem("gia_chat_history");
    if (saved) {
      try {
  ...
  useEffect(() => {
    ...
    localStorage.setItem("gia_chat_history", JSON.stringify(messages));
  }, [messages]);
  ...
  const handleTextCommand = useCallback(async (finalTranscript: string) => {
    ...
    if (commandResult.isBrowserAction) {
      ...
      setMessages((prev) => [...prev, { id: Date.now().toString() + "-g", sender: "gia", text: responseText }]);
      
      if (!isMuted) {
        setAppState("speaking");
        const audioBase64 = await getGiaAudio(responseText);
        if (audioBase64) {
      ...
      responseText = await getGiaResponse(finalTranscript, messagesRef.current);
      setMessages((prev) => [...prev, { id: Date.now().toString() + "-g", sender: "gia", text: responseText }]);
    ...
    <div className="h-[100dvh] w-screen bg-[#050505] text-white flex flex-col items-center justify-between font-sans relative overflow-hidden m-0 p-0">
      ...
      <header className="absolute top-0 left-0 w-full flex justify-between items-center z-20 shrink-0 px-6 py-4 md:px-12 md:py-6">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-xl font-serif font-medium tracking-wide opacity-90">Gia</h1>
        </div>
          ...
          {showTextInput && (
            ...
            >
              <input 
                ...
                placeholder="Type a message to Gia..."
